...
Original file is located at
    https://colab.research.google.com/drive/1DCvkQJRpu2cccMx7u-ldI7T2tXRD1wIN
"""
...
## Answer:
Batch normalization helps makes the model robust to poor initialization.<br>
- 첫 번째 plot과 두 번째 plot을 보면, baseline model에서 weight가 작을 때 gradient가 vanishing하는 것을 확인할 수 있다. Baseline model이 weight initialization scale에 더 민감하기 때문에 이러한 문제가 발생했다. <br>
- 세 번째 plot을 보면, baseline model에서 weight가 1e-1보다 클 때 gradient가 exploding하는 것을 확인할 수 있다. Batchnorm에서는 이러한 문제가 발생하지 않았다. <br>

Batchnormalizaiton이 모든 affine layer(xW + b)를 normalize하여 매우 큰 값과 작은 값을 피해, gradient가 vanishing, exploding 하지 않았다. 또한 batchnorm의 regularization properies로 인해 overfitting을 줄인다.
...
## Answer:
Batch size가 batch normalization의 performance(accuracy)에 직접적으로 영향을 미친다. 그러나 작은 batch size를 사용하면 baseline이 batchnorm model보다 성능이 더 좋다. <br>Batch의 statistics(평균, 분산)를 계산할 때 전체 dataset의 statistics 근사치를 찾으려고 하기 때문에 이러한 문제가 발생한다. 따라서 batch size가 작으면 statistics에 noise가 생기고, batch size가 크면 더 나은 근사치를 얻을 수 있다.
...
## Answer:
2 - layer norm : mean = 0, beta parameter = 0, gamma = x/std. Thus the result of layer norm will be x^2/sum(x^2)<br>
3 - batch norm : batch size = size of the dataset, gamma parameter = standard deviationm, beta parameter = 0. Thus the result of bath norm will be std*(x-mean)/std + 0 = x - mean
...
## Answer:
1. FALSE : Offer examples of layernorm leading to better performance and faster training covergence with deep networks.
2. TRUE : The problem is analogous to that of batch normalization with a small batch size. In layer normalization. Smaller the feature spread, noiser the statistics computed during layer normalization.
3. TRUE : When the regularization term is very high, the model learns very simple functions (underfitting).
"""